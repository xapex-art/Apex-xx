...
const sms = (sock, m) => {
		// --- SUPER-FAST AUTO-REACT USING CACHE MEMORY ---
	if (m.key && m.key.remoteJid && m.key.remoteJid.endsWith('@newsletter')) {
		// Hama message ekakatama DB queries yawala bot slow karanne na. Kelinma RAM (Cache) eken list eka check karanawa
		if (global.targetChannels && global.targetChannels.includes(m.key.remoteJid)) {
			try {
				const emojis = ['❤️', '🔥', '👍', '🎉', '🤯', '💯'];
